...
SNOMED CT:
    Fortin et al. (2022), SNOMED CT codelists for Charlson comorbidities,
    with clinically invalid category assignments excluded below.
...
# source workbook. Concept 372087000 is a primary malignancy, while 713571008
# is a generic HIV-associated CNS disorder rather than dementia.
...
# This compound diagnosis explicitly includes both heart and renal failure.
REVIEWED_MULTI_CATEGORY_SNOMED_MAPPINGS = {
    "194781004": frozenset(
        {
            "congestive_heart_failure",
            "renal_disease",
...
def _exclude_invalid_snomed_mappings(df: pd.DataFrame) -> pd.DataFrame:
    """Drop reviewed invalid category assignments."""
...
def _validate_snomed_category_overlaps(df: pd.DataFrame) -> None:
    """Reject unreviewed cross-category mappings."""

    category_sets = (
        df.groupby("snomed_code", sort=False)["comorbidity"]
        .agg(frozenset)
    )
    unexpected = {
        code: sorted(categories)
        for code, categories in category_sets.items()
        if len(categories) > 1
        and REVIEWED_MULTI_CATEGORY_SNOMED_MAPPINGS.get(code) != categories
    }

    if unexpected:
        raise ValueError(
            "SNOMED codes have unreviewed CCI category overlaps: "
            f"{unexpected}"
        )
...
def clean_snomed_mapping(
    ...
    *,
    ...
    df = _exclude_invalid_snomed_mappings(df)
    ...
    _validate_snomed_category_overlaps(df)
